...
## 简介
本文件面向FH6汽车设置工具的AI集成模块，系统性阐述AI提供商配置机制、提示词系统设计、工具调用流程、流式响应（SSE）实现与客户端连接管理、AI模型配置与参数调优策略、性能优化建议以及故障排除与错误处理最佳实践。该模块以Express作为后端框架，通过统一的AI提供商抽象接入多种OpenAI兼容的服务，并以流式Server-Sent Events（SSE）向客户端推送AI响应。

## 项目结构
后端采用分层组织：入口与应用初始化、路由层、AI能力封装（提供商、提示词、工具、模式定义）、共享类型定义等。前端通过HTTP接口与后端交互，后端再通过AI SDK对接外部模型服务。
...
### AI提供商配置机制
- 支持的提供商类型：通过共享类型定义了多种提供商类型，便于扩展与切换。
- 配置项：包含基础URL、API密钥、模型名、温度、最大Token数等。
...
+type : "deepseek" | "openai" | "ollama" | "custom"
+baseURL : string
+apiKey : string
+model : string
+temperature : number
...
AIProviderConfig <|-- DEFAULT_AI_CONFIG : "默认值"
```
...
- 自定义提示词：可通过扩展提示词模板或在消息上下文中注入额外指令，实现场景化定制。
...
### 工具调用机制
- 工具定义：工具集定义了可被AI调用的操作集合，用于将AI建议转化为具体调优参数。
...
ToolResult --> TuningParameters : "映射为调参"
```

**图表来源**
- [shared/types/chat.ts:27-38](file://shared/types/chat.ts#L27-L38)
- [shared/types/tuning.ts:68-78](file://shared/types/tuning.ts#L68-L78)
...
**章节来源**
- [server/src/routes/chat.ts:17-24](file://server/src/routes/chat.ts#L17-L24)
- [shared/types/chat.ts:10-12](file://shared/types/chat.ts#L10-L12)
...
- 外部SDK：通过AI SDK进行流式文本生成与工具调用，简化跨提供商的适配。
...
## 结论
本AI集成模块通过统一的提供商抽象、结构化的提示词与工具集、以及SSE流式推送，实现了对多提供商OpenAI兼容服务的灵活接入与高效调用。配合完善的类型定义与错误处理机制，能够在保证易用性的同时满足复杂调优场景的需求。建议在生产环境中进一步完善缓存、限流与监控体系，持续优化生成质量与响应性能。
