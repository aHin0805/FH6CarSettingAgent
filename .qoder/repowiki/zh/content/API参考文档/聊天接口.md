...
## 简介
本文件为 POST /api/chat 聊天接口的完整技术文档，涵盖接口的 HTTP 方法与 URL、请求与响应格式、messages 数组参数结构、流式 SSE 响应机制、sessionId 参数作用、AI 提供商配置对接口行为的影响、典型汽车调优咨询场景的请求/响应示例、错误处理机制（400、401、500），以及 maxSteps 参数与工具调用机制。

**更新** 本版本反映了基于Next.js客户端架构的新实现，支持现代化的流式响应和SSE连接管理。

## 项目结构
该接口位于前后端分离的架构中，客户端使用Next.js框架，服务端提供RESTful API。系统包含客户端聊天页面、共享类型定义、AI 提供商封装、系统提示词与工具集等模块。
...
subgraph "服务端"
C["Express 应用<br/>server/src/app.ts"]
D["聊天路由<br/>server/src/routes/chat.ts"]
E["AI 提供商封装<br/>server/src/ai/provider.ts"]
F["系统提示词<br/>server/src/ai/prompts/system.ts"]
G["工具集<br/>server/src/ai/tools.ts"]
H["共享类型定义<br/>shared/types/chat.ts"]
end
A --> B
B --> C
C --> D
...
**图表来源**
- [client/src/app/(main)/chat/page.tsx:1-100](file://client/src/app/(main)/chat/page.tsx#L1-L100)
...
## 核心组件
- 客户端聊天页面：基于Next.js App Router构建，提供现代化的聊天界面和流式响应处理
- 聊天模型定义：定义消息格式、工具调用和响应处理的数据结构
...
## 架构总览
POST /api/chat 的调用链路采用现代Next.js架构，客户端通过fetch API发起请求，服务端通过Express应用处理，最终返回流式SSE响应：
...
**图表来源**
- [client/src/app/(main)/chat/page.tsx:45-100](file://client/src/app/(main)/chat/page.tsx#L45-L100)
...
**更新** 客户端通过Next.js的App Router架构实现现代化的聊天体验，支持实时流式响应和SSE连接管理。
...
**更新** 客户端通过Next.js的现代化架构实现更流畅的请求/响应体验。
...
**更新** 新架构利用Next.js的现代化特性提供更好的性能表现。
...
## 结论
POST /api/chat 接口通过流式 SSE 提供实时对话体验，结合系统提示词与工具集实现 FH6 车辆调校的专业问答。新的Next.js客户端架构提供了现代化的用户体验，支持高效的流式响应和SSE连接管理。通过 sessionId 支持会话管理，借助 AI 提供商配置灵活适配不同服务端点与模型。合理的错误处理与性能优化保障了生产可用性。
